...
const SettingsPage = () => {
  ...
  useEffect(() => {
    const fetchKeys = async () => {
      try {
        // const response = await fetch('http://127.0.0.1:8084/api/get-keys/');
        // Use the environment variable for the API call
        const response = await fetch(`${API_URL}/api/get-keys/`);
        if (response.ok) {
  ...
  const handleSaveKeys = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/save-keys/`, {
        method: 'POST',
        headers: {
